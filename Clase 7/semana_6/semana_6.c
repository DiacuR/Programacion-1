    ...
    for(i = 0; i < tamProd; i++)
    {
        if(listaProductos[i].estado == 1)
        {
            ...
            for(j = 0; j < tamProv; j++)
            {
                ...
                {
                    printf("%s %s %f %s", listaProductos.codigoDeBarra, listaProductos.nombre,
                            listaProductos.precio, listaProveedor[j].duenio);
                    break;
                }
            }
...
void mostrarProveedorConSusProductos(eProveedor listadoProveedores[] ,int tamProv, eProducto listaProductos[], int tamProd)
{
    int i, j, contadorProductos;

    for( i = 0; i < tamProv; i++)
    {
        contadorProductos = 0;
        if(listadoProveedores[i].estado == 1)
        {

            for(j = 0; j < tamProd; j++)
            {
                if(listadoProveedores[i].id == listaProductos[j].idProveedor)
                {
                    contadorProductos++;
                }
            }
